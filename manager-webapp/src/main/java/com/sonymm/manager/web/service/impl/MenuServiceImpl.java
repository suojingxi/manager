...
import com.sonymm.manager.web.utils.Constants;
import com.sonymm.manager.web.utils.ResourceTreeUtil;
...
public class MenuServiceImpl implements MenuService {
    ...
    public void add(Menu menu) {
        ...
        checkArgument(Constants.STATUS_SET.contains(menu.getIsShow()), "菜单是否显示不正确");
        checkArgument(Constants.STATUS_SET.contains(menu.getType()), "菜单类型不正确");
        if(menu.getType() == 1){
        ...
        if(!Strings.isNullOrEmpty(menu.getParentId()) && !"0".equals(menu.getParentId())){
            ...
            menu.setParentName(parentMenu.getParentName());
        }
    ...
    private List<MenuTitle> loadResources(String roleId){
        ...
        list = new ArrayList<MenuTitle>(8);
    ...
    public List<Menu> getList(ParamFilter param) {
        return menuDao.find("getMenuList", param.getParam(), param.getPage());
    }
